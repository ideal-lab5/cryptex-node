...
WORKDIR /cryptex-node
COPY . .
RUN cargo build --locked --release

# This is the 2nd stage: a very small image where we copy the binary."
FROM docker.io/library/ubuntu:20.04
LABEL description="Multistage Docker image for Cryptex Node" \
  image.type="builder" \
  image.authors="driemworks@idealabs.network" \
  image.vendor="Ideal Labs" \
  image.description="Multistage Docker image for Cryptex node" \
...
COPY --from=builder /cryptex-node/target/release/node-template /usr/local/bin
